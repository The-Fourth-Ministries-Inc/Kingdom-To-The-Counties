...
## Pre-Crusade no longer OOMs Chrome (v1.19.11)
...
on one tick. Opening the tab writes “Opening the church list…” and
waits **two animation frames** so Chrome paints Mobilization before
any roster work (`setTimeout(0)` still ran before paint and looked
like Event Day had frozen). Stats paint first; the list and change
log wait a tick. Conversation lookups are indexed once per paint.
The `k2c_churches` cache is parsed lazily, not at script load, and
paint errors are caught so a bad row cannot discard the tab. The
roster is fetched only when Pre-Crusade or a church card opens —
not on first paint of Event Day. Specialists is a static hub; Mike’s
freeze there was the same Chrome process already dying.
...
**Counties we've already been to drop off the board by themselves (v1.17.0).**
...
the **county/event** is a dropdown of the counties still ahead (plus "Other / custom"),
...
- **The Recording Studio board self-seeds — and self-retires (v1.17.0).**
